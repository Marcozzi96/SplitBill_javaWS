...
|-----------------------|------------------------------------------------------------------------|------------------------------------------------------|
| `JWT_SECRET`          | Chiave segreta per la firma dei token JWT (minimo 256 bit consigliati) | valore di default in `application-dev.yml`           |
| `JWT_VALIDITY`        | Durata di validità del token in secondi                                | `86400` (24 ore)                                     |
| `OPEN_LINK`           | URL base del frontend, usato nel link di conferma registrazione        | `http://localhost:3000`                               |
...
| `JWT_SECRET`                  | Chiave segreta per la firma dei token JWT                              | una stringa lunga e casuale                          |
| `JWT_VALIDITY`                | Durata di validità del token in secondi                                | `86400` (24 ore)                                     |
...
2. **Configura le variabili d'ambiente (opzionale in dev)**
   ...
   In dev il backend parte con H2 e valori di default. Puoi comunque sovrascrivere `JWT_SECRET` e `OPEN_LINK`:

   ```bash
   export JWT_SECRET=supersecretkey123supersecretkey123
...
| POST   | `/auth/register`      | Registrazione nuovo utente + invio email     |
